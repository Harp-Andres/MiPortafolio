...
class LinterChecker(BaseSkill):
    ...
    async def _run_implementation(self, request) -> SkillResult:
        ...
        try:
            ...
            issues = await self._check_lints(params)
            duration = (datetime.now() - start_time).total_seconds() * 1000
            
            status = SkillStatus.SUCCESS if issues == 0 else SkillStatus.FAILED
            return SkillResult(
                skill_name=self.skill_name,
                status=status,
                output={"lint_issues": issues, "duration_ms": duration},
            )
    ...
    async def _check_lints(self, params: dict) -> int:
