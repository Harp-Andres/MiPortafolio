"""Synchronization verification service - Implementation of SyncValidator interface"""

from datetime import datetime
from typing import Dict, List, Any

from app.domain.entities.cv_models import CVDataModel, SyncReportModel
from app.domain.use_cases.sync_validator import SyncValidator as SyncValidatorInterface


class SyncValidator(SyncValidatorInterface):
    ...
    def validate(self, cv_data: Dict[str, Any] = None) -> SyncReportModel:
        """
        Run complete validation.
        
        Args:
            cv_data: Optional CV data dict (uses instance data if not provided)
        ...
        if cv_data is not None:
            try:
                self.cv_data = CVDataModel(**cv_data)
                self.mismatches = []
